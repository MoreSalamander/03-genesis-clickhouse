#!/usr/bin/env bash
# One command for Institutional Intelligence: API + console together.
#
#   ./ops/dev.sh          → http://localhost:3020
#
...
if nc -z localhost "${TEMPORAL_PORT:-7235}" 2>/dev/null; then
  echo "→ worker  connected to Temporal :${TEMPORAL_PORT:-7235}"
  .venv/bin/python -m app.workflows.worker &
else
  echo "→ worker  skipped (no Temporal on :${TEMPORAL_PORT:-7235}) — the API runs the loop in-process"
fi
